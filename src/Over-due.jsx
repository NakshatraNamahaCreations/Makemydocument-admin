...
function OverDue({selectedItem}) {
  ...
  const leadsPerPage = 10; // Showing only 7 leads per page
...
console.log(adminData.name);
  ...
  useEffect(() => {
    const fetchLeads = async () => {
      try {
        if (!adminData?.name) {
          throw new Error("Admin name is missing in localStorage.");
        }
  
        const data1 = adminData.name
  
        const response = await fetch(
          `${process.env.REACT_APP_API_URL}/api/getOverdueLead?assign=${adminData.name}`,
          {
            method: "GET",
            headers: {
  ...
  return (
    <>
     <Helmet>
    <title>OverDue - Make my Documents</title>


<meta name="author" content="https://leads.makemydocuments.in/over-due"/>

...
function Converted({selectedItem}) {
  ...
  useEffect(() => {
    const fetchLeads = async () => {
      try {
        // Check if adminData exists and has a name
        if (!adminData || !adminData.name) {
          throw new Error("Admin data is missing or invalid in localStorage.");
        }
        ...
        const response = await fetch(
          `${process.env.REACT_APP_API_URL}/api/get-converted-leads?assign=${adminData.name}`,
          {
            method: "GET",
            headers: {
        ...
        if (Array.isArray(data)) {
          const reversedLeads = data.reverse(); // Reverse the leads
          setLeads(reversedLeads); // Update state with the reversed data
        } else if (data && data.status === "success" && Array.isArray(data.data)) {
          const reversedLeads = data.data.reverse(); // Reverse nested leads
          setLeads(reversedLeads);
        } else {
          throw new Error("Invalid data format received from the server.");
        }
      } catch (error) {
        console.error("Error fetching converted leads:", error.message);
  ...
  return (
    <>
     <Helmet>
    <title>Converted - Make my Documents</title>


<meta name="author" content="https://leads.makemydocuments.in/converted"/>


    </Helmet>
    <div className="new-leads-container d-none d-lg-block"   style={{width: "470%", marginLeft: "235px", marginTop: "106px" ,  fontFamily:"Poppins, sans-serif", }}>
       {!selectedLead ? (
      <div>
        <div
            style={{
        ...
        {filteredLeads.map((lead, index) => (
           ...
           >
            ...
            <td   style={{ ...styles.tableCell, whiteSpace: 'nowrap' }} onClick={() => handleRowClick(lead)}>{lead.date}</td>
              <td style={styles.tableCell} onClick={() => handleRowClick(lead)}>{lead.name}</td>
              <td style={styles.tableCell} onClick={() => handleRowClick(lead)}>{lead.mobilenumber}</td>
              <td style={styles.tableCell} onClick={() => handleRowClick(lead)}>{lead.service}</td>
              <td style={styles.tableCell} onClick={() => handleRowClick(lead)}>{lead.district}</td>
